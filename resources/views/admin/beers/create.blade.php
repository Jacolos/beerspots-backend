        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <form action="{{ isset($beer) ? route('admin.beers.update', $beer) : route('admin.beers.store') }}"
                ...
                <div>
	                ...
	                <select name="type" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-500 focus:border-transparent" required>
	                    <option value="">Wybierz typ piwa</option>
	                    <option value="lager" {{ old('type') == 'lager' ? 'selected' : '' }}>Lager</option>
	                    <option value="pilsner" {{ old('type') == 'pilsner' ? 'selected' : '' }}>Pilsner</option>
	                    <option value="ale" {{ old('type') == 'ale' ? 'selected' : '' }}>Ale</option>
	                    <option value="ipa" {{ old('type') == 'ipa' ? 'selected' : '' }}>IPA</option>
	                    <option value="stout" {{ old('type') == 'stout' ? 'selected' : '' }}>Stout</option>
	                    <option value="porter" {{ old('type') == 'porter' ? 'selected' : '' }}>Porter</option>
	                    <option value="wheat" {{ old('type') == 'wheat' ? 'selected' : '' }}>Pszeniczne</option>
